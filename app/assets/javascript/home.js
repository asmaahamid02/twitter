...
function createProfileToFollow(user_id, name, username, profile_img) {
  ...
  name_link.href = `${base_url}/app/views/profile.html?id=${user_id}`
  ...
  follow_btn.addEventListener('click', () => {
    ...
    fetch_api(api + 'follow_user.php', data_obj)
      .then((data) => {
        if (data.success) {
          follow_btn.textContent = data.success
...
fetch(`${api}get_random_users.php?id=${id}`)
  .then((res) => res.json())
  .then((data) => {
    if (!data.error || !data.empty || data.length > 0) {
      // console.log(data)
      profilesToFollowLoop(data)
    } else {
      console.log(data.empty ? data.empty : data.error)
    }
  })
  .catch((error) => console.log(error))
