...
if (isset($_POST['username']) && isset($_POST['publicity']) && (isset($_POST['tweet']) || isset($_POST['picture']))) {
    ...
    $image = '';
    if ($picture != '') {
        $image_data = convertToBase64($picture, '../../app/assets/images/tweets_imgs/', 'assets/images/tweets_imgs/');
    ...
    if ($result_insert) {
        ...
        if ($picture != '') {
            file_put_contents($image_data['folder_save'], $image_data['base64string']); //add image to the folder
        }
    } else {
